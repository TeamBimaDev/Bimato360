...
urlpatterns = [
    ...
    path('<str:public_id>/ajoutaddress/', BimaCompanyViewSet.as_view({'post': 'ajout_address_for_company'}), name='ajout-address-company'),
    path('<str:public_id>/document/', BimaCompanyViewSet.as_view({'get': 'list_documents'}), name='document-addresses'),
    path('<str:public_id>/ajoutdocument/', BimaCompanyViewSet.as_view({'post': 'ajout_document_for_company'}),
