...
class BimaHrOffreViewSet(viewsets.ModelViewSet):
    ...
    action_permissions = {
        ...
        'description': ['offre.can_generate_description'],
    }
    ...
    def list_bimahr_offres(request):
        offres = BimaHrOffre.objects.all().order_by('-created_at')  
        serializer = BimaHrOffreSerializer(offres, many=True)
        return JsonResponse(serializer.data, safe=False)

    def get_object(self):
            obj = BimaHrOffre.objects.get_object_by_public_id(self.kwargs['pk'])
            return obj
...
'''class BimaHrOffreViewSet(viewsets.ModelViewSet):
    ...
    def create_offre(self, request):
        ...
        # Generate description for the new offre
        generated_content = self.generate_job_description(offre)
        offre.generated_content = generated_content
        offre.save()
