...
class BimaHrVacationViewSet(AbstractViewSet):
    ...
    def manage_vacation(self, request, pk=None):
        ...
        if str(vacation.manager.public_id) != manager_public_id:
            raise PermissionDenied("You are not authorized to manage this vacation.")
        ...
        with transaction.atomic():
            if status_update == VacationStatus.APPROVED.value:
                ...
                if not is_valid:
                    return Response({
                        'error': f'Requested vacation days exceed available balance by {requested_days - vacation.employee.balance_vacation} days.'},
