...
class TestBimaTreasuryPaymentTermAPI:
    @pytest.fixture
    def model(self, model_data):
        return BimaTreasuryPaymentTerm.objects.create(**model_data)

    def test_create_model(self, api_client, model_data):
        url = reverse('BimaTreasuryPaymentTerm-list')
        response = api_client.post(url, data=model_data, format='json')
        assert response.status_code == status.HTTP_201_CREATED
        assert BimaTreasuryPaymentTerm.objects.filter(**model_data).exists()

    def test_retrieve_model(self, api_client, model):
        url = reverse('BimaTreasuryPaymentTerm-detail', kwargs={'pk': model.id})
        response = api_client.get(url)
        assert response.status_code == status.HTTP_200_OK
        assert response.data == BimaTreasuryPaymentTermSerializer(model).data
    ...
    def test_delete_model(self, api_client, model):
        ...
        assert not BimaTreasuryPaymentTerm.objects.filter(id=model.id).exists()
