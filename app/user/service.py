...
def verify_user_credential_when_change_password(uidb64, token, public_id):
    ...
    try:
        ...
        return {"detail": _('Unable to identify credential')}, status.HTTP_400_BAD_REQUEST
    ...
    if timezone.now() - user.reset_password_time > timedelta(hours=24):
        return {"detail": _('Your token has expired please contact the administrator')}, status.HTTP_400_BAD_REQUEST
